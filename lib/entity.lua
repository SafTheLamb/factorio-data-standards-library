...
function fds_entity.get_resistances(entity_type, entity_name)
	return
end

---Gets the resistance a given prototype has.
---@param entity_type string
---@param entity_name string
---@param damage_type string
---@return data.Resistance
function fds_entity.get_resistance(entity_type, entity_name, damage_type)
	if entity and entity.resistances then
		for i,resistance in pairs(entity.resistances) do
			if resistance.type == damage_type then
				return entity.resistances[i]
			end
		end
...
---Sets the damage resistance for a given prototype.
...
function fds_entity.add_resistance(entity_type, entity_name, add_resistance)
	local entity = data.raw[entity_type][entity_name]
	if entity and entity.resistances then
		for i,resistance in pairs(entity.resistances) do
			if resistance.type == damage_type then
